...
def test_check_type_positive(mocker, func, test_value, expected):
    ...
    assert test_func(mock_type_check, test_value) == expected
...
def test_check_type_str_err_negative(mocker):
    ...
    with pytest.raises(TypeError) as exc:
        AnsibleModule._check_type_str(mock_type_check, [])
...
def test_check_type_list_negative(mocker):
    mock_type_check = mocker.MagicMock()
    with pytest.raises(TypeError) as exc:
        AnsibleModule._check_type_list(mock_type_check, {})
...
def test_check_type_dict_negative(mocker):
    mock_type_check = mocker.MagicMock()
    with pytest.raises(TypeError) as exc:
        AnsibleModule._check_type_dict(mock_type_check, '1')
...
def test_check_type_bool_negative(mocker):
    mock_type_check = mocker.MagicMock()
    with pytest.raises(TypeError) as exc:
        AnsibleModule._check_type_bool(mock_type_check, [])
...
def test_check_type_int_negative(mocker):
    mock_type_check = mocker.MagicMock()
    with pytest.raises(TypeError) as exc:
        AnsibleModule._check_type_int(mock_type_check, 'aaa')
...
def test_check_type_float_negative(mocker):
    mock_type_check = mocker.MagicMock()
    with pytest.raises(TypeError) as exc:
        AnsibleModule._check_type_float(mock_type_check, 'aaa')
...
def test_check_type_jsonarg_negative(mocker):
    mock_type_check = mocker.MagicMock()
    with pytest.raises(TypeError) as exc:
        AnsibleModule._check_type_jsonarg(mock_type_check, True)
...
def test_check_type_bytes_negative(mocker):
    ...
    with pytest.raises(TypeError) as exc:
        AnsibleModule._check_type_bytes(mock_type_check, True)
